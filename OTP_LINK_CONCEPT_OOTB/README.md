...
Out-of-the-box extension of Recharge's turnkey Subscription Widget (no forked/headless widget). It produces a **subscription-first** layout: selling plans render as stacked cards, and the one-time purchase option appears last as a quiet link that adds the item to the cart via AJAX.
...
1. Paste `recharge-custom-widget-styling` into: Merchant portal → Cross-Sell & Upsell → Subscription widget → Customize → Edit styles → Advanced.
